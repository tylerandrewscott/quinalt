...
time.plot = ggplot()+geom_path(aes(x=mod3$summary.random$total.period$ID,
  ...
  scale_x_continuous('Time Period',breaks= seq(1,228,12),labels=c(1995:2013),expand=c(0,0))

ggsave(filename='time.trend.plot.png',time.plot,width=6,height=4,units='in')
