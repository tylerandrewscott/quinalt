...
print(xtable(exp(mod.1yr.rest.nonrest$summary.fixed),digits=3),type='html','mod.rest.nonrest.1yr.table.html')
print(xtable(exp(mod.3yr.rest.nonrest$summary.fixed),digits=3),type='html','mod.rest.nonrest.3yr.table.html')
print(xtable(exp(mod.5yr.rest.nonrest$summary.fixed),digits=3),type='html','mod.rest.nonrest.5yr.table.html')

dic.scores.rest.non.rest = data.frame(mod.1yr.rest.nonrest$dic$dic,
                                   mod.3yr.rest.nonrest$dic$dic,
                                   mod.5yr.rest.nonrest$dic$dic)
colnames(dic.scores.rest.non.rest) = c(paste('Model ',c('1yr','3yr','5yr'),sep=''))
dic.scores.rest.non.rest[1,] = round(dic.scores.rest.non.rest[1,])

write.table(dic.scores.rest.non.rest,'H:/quinalt/dic.score.rest.non.rest.table.txt')
