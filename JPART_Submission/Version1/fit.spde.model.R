...
#Model 0: No spatial effect
form0.covar <-  y ~ 0 + b0 + Ag + Forst + Dev  + elevation + seaDist + rsum
  f(HUC8,model='iid') + f(total.period,model='rw2') + f(seasonal,model='seasonal',season.length=12)

mod0.covar <- inla(form0, family='gaussian', 
             data=data.frame(y=temp$l.owqi, covars,b0=1), 
             control.predictor=list(compute=TRUE),
             #     control.inla=list(strategy='laplace'), #note that we are here using laplace, default in R-INLA is the simplified laplace approximation (run faster)
             control.compute=list(dic=TRUE, cpo=TRUE))
...
dic.scores = data.frame(mod0$dic$dic,mod1$dic$dic,
mod2$dic$dic,
mod3$dic$dic,
mod4$dic$dic,
mod5$dic$dic,
mod6$dic$dic,
mod7$dic$dic)
colnames(dic.scores) = c(paste('Model ',0:7,sep=''))
dic.scores[1,] = round(dic.scores[1,])

write.table(dic.scores,'H:/quinalt/dic.score.table.txt')

#model 3 performs far, far better

stargazer(data.frame(mod3$summary.fixed),type='html',summary=F,out='H:/quinalt/basemode.summary.table.html')
