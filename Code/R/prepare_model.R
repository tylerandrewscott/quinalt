...
params.spdf@data = params.spdf@data %>% 
  mutate(fund.bin.1yr = ifelse(rsum.oweb.all.1yr>0,1,0))%>%
  mutate(fund.bin.3yr = ifelse(rsum.oweb.all.1yr>0,1,0))%>%
  mutate(fund.bin.5yr = ifelse(rsum.oweb.all.1yr>0,1,0))%>%
...
#mesh.or = inla.nonconvex.hull(loc=cbind(coordinates.or$LONG,coordinates.or$LAT),
...
save.image('midpoint.7.v2.RData')
