...
# ggsave(filename = 'Deliverables/JPART/Final/figure4.eps',p4,width=w,height=he,dpi = 900,fonts=c("serif", "Palatino"),units = 'in')


temp.or = readOGR(dsn='SpatialData/government_units','state_nrcs_a_or')

or.fort = fortify(temp.or)

mesh.locs = data.frame(mesh.a$loc)
mesh.locs$id = 1:nrow(mesh.locs)
bounds = mesh.a$segm$bnd
bounds = data.frame(bounds$idx)
bounds$X = mesh.locs$X1[match(bounds$X1,mesh.locs$id)]
bounds$Xend = mesh.locs$X1[match(bounds$X2,mesh.locs$id)]
bounds$Y = mesh.locs$X2[match(bounds$X1,mesh.locs$id)]
bounds$Yend = mesh.locs$X2[match(bounds$X2,mesh.locs$id)]


seg = data.frame(mesh.a$segm$int$idx)
seg$X = mesh.locs$X1[match(seg$X1,mesh.locs$id)]
seg$Xend = mesh.locs$X1[match(seg$X2,mesh.locs$id)]
seg$Y = mesh.locs$X2[match(seg$X1,mesh.locs$id)]
seg$Yend = mesh.locs$X2[match(seg$X2,mesh.locs$id)]


triangles = data.frame(mesh.a$graph$tv)
colnames(triangles) = c('X','Y','Z')
first.coord = cbind(mesh.locs$X1[match(triangles$X,mesh.locs$id)],mesh.locs$X2[match(triangles$X,mesh.locs$id)])
second.coord = cbind(mesh.locs$X1[match(triangles$Y,mesh.locs$id)],mesh.locs$X2[match(triangles$Y,mesh.locs$id)])
third.coord = cbind(mesh.locs$X1[match(triangles$Z,mesh.locs$id)],mesh.locs$X2[match(triangles$Z,mesh.locs$id)])

vertex.locs = data.frame(rbind(cbind(first.coord,second.coord),
cbind(first.coord,third.coord),
cbind(second.coord,third.coord)))
colnames(vertex.locs) = c('X','Y','Xend','Yend')

obs.locs = data.frame(X=mod.data$long,Y=mod.data$lat)

ggplot() + geom_segment(aes(x=X,xend=Xend,y=Y,yend=Yend),data=bounds,colour="#E69F00") + 
  geom_polygon(aes(x=long,y=lat,group=group),data=or.fort,fill="#999999") +
  geom_segment(aes(x=X,xend=Xend,y=Y,yend=Yend),data=seg,colour="#E69F00")  +
  geom_segment(aes(x=X,xend=Xend,y=Y,yend=Yend),data=vertex.locs,colour="#009E73")  + 
  geom_point(aes(x=X,y=Y),data=obs.locs,colour='#56B4E9') +
  theme_bw() +
  theme_tufte(ticks=F) +
